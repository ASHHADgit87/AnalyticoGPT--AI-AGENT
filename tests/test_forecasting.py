...
def test_unit12_compute_linear_extrapolation_includes_model_coefficients(series_dataframe):
    results = compute_linear_extrapolation(series_dataframe, "Hours_Studied", "Score", steps=3)
    assert "coefficient_slope" in results
    assert "y_intercept" in results

def test_unit13_compute_linear_extrapolation_calculates_positive_trend(series_dataframe):
    results = compute_linear_extrapolation(series_dataframe, "Hours_Studied", "Score", steps=3)
